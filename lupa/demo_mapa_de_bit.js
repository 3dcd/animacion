	...
	this.frame_48 = function() {
		...
		*/
		...
		function iniciarIdle() {
			...
			function ciclo() {
				...
				createjs.Tween.get(posDeseada, { override: true })
					.wait(700)
					.to({ x: baseX + 85, y: baseY + 25 }, 1100, createjs.Ease.sineInOut)
					.wait(250)
					.to({ x: baseX - 55, y: baseY - 20 }, 1200, createjs.Ease.sineInOut)
					.wait(250)
					.to({ x: baseX + 25, y: baseY + 55 }, 1000, createjs.Ease.sineInOut)
					.wait(250)
		...
		createjs.Ticker.on("tick", function() {
			posSuave.x += (posDeseada.x - posSuave.x) * 0.42;
			posSuave.y += (posDeseada.y - posSuave.y) * 0.42;
...
an.makeResponsive = function(isResp, respDim, isScale, scaleType, domContainers) {
	...
	function resizeCanvas() {
		...
		// Capo el pixel ratio para que en celular no se vuelva pesado/tosco.
		var pRatio = Math.min(window.devicePixelRatio || 1, 2);
		...
		var cssW = Math.round(w * sRatio);
		var cssH = Math.round(h * sRatio);
		var pxW = Math.round(w * pRatio * sRatio);
		var pxH = Math.round(h * pRatio * sRatio);

		domContainers[0].width = pxW;
		domContainers[0].height = pxH;
